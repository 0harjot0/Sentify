...
class DataTransformation:
    def __init__(self, config: DataTransformationConfig):
        ...
        '''
        self.config = config 
        create_directories(self.config.embedding_paths)
    ...
    def __get_bert_embeddings(self, sequences):
        ...
        for seq in sequences:
            ...
            count += 1
            if count % 100 == 0:
                logger.info(f"{count} {embeddings_merged.shape}")
    ...
    def __save_fasttext_models(self):
        ...
        self.fasttext_model.save(self.config.fasttext_model)
    ...
    def __prepare_dataset(self):
        ...
        self.X_train_raw = raw_train['text'].tolist()
        self.y_train = raw_train['target'].to_numpy()
        logger.info(f"Train data shape - {raw_train.shape}")
        
        raw_valid = pd.read_csv(self.config.data_path + "valid.csv")
        self.X_valid_raw = raw_valid['text'].tolist()
        self.y_valid = raw_valid['target'].to_numpy()
        logger.info(f"Validation data shape - {raw_valid.shape}")
        
        raw_test = pd.read_csv(self.config.data_path + "test.csv")
        self.X_test_raw = raw_test['text'].tolist()
        self.y_test = raw_test['target'].to_numpy()
        logger.info(f"Test data shape - {raw_test.shape}")
