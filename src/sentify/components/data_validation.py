...
class DataValidation:
    ...
    def pre_process_validation(self):
        ...
        if os.path.exists(self.config.pre_process_file):
            logger.info(f"Data file validated for the processing at {self.config.pre_process_file} of size {get_size(Path(self.config.pre_process_file))}")
            
        else: 
            raise Exception(f"Data file wasn't validated for processing at {self.config.pre_process_file}")
    ...
    def post_process_validation(self):
        ...
        logger.info(f"Data files validated for the post processing at {self.config.data_path}")
